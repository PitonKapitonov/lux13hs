...
module udp_stack #(
    ...
    mac_fifo tx_mac_fifo(
        ...
        .din ({    tx_fifo_tlast,     tx_fifo_tuser,     tx_fifo_tdata}), 
        .rd_clk(tx_axis_mac_aclk), 
        .empty(tx_fifo_empty), 
        .rd_en(tx_axis_mac_tready), 
        .dout({tx_axis_mac_tlast, tx_axis_mac_tuser, tx_axis_mac_tdata})
    );
    ...
    mac_fifo rx_mac_fifo(
        .wr_clk(rx_axis_mac_aclk), 
        .full(),               
        .wr_en(rx_axis_mac_tvalid), 
        .din ({rx_axis_mac_tlast, rx_axis_mac_tuser, rx_axis_mac_tdata}), 
        .rd_clk(clk),              
        .empty(rx_fifo_empty), 
        .rd_en(rx_fifo_tready),     
        .dout({    rx_fifo_tlast,     rx_fifo_tuser,     rx_fifo_tdata})
    );
    ...
    logic  arp_rx_dv_out;
    logic[47:0] remote_mac = 48'h00_da_db_01_02_03;
    logic[31:0] remote_ip = 32'h10_00_00_01;  
    ...
    arp_rx #(.local_mac(local_mac), .local_ip(local_ip), .local_port(local_port)) arp_rx_inst (
        ...
        .rx_fifo_tuser(rx_fifo_tuser),  // connect to rx fifo
        .dv_out(arp_rx_dv_out), 
        .remote_mac(remote_mac), 
        .remote_ip(remote_ip),
        .udp_tvalid(udp_rx_tvalid), 
        .udp_tdata(udp_rx_tdata), 
        .udp_tlast(udp_rx_tlast), 
        .udp_tuser(udp_rx_tuser)
    );


    // send arp replies and format udp doble_row_bots for transmit
    ...
    arp_tx #(.local_mac(local_mac), .local_ip(local_ip), .local_port(local_port), .remote_port(remote_port)) arp_tx_inst (
        ...
        .tx_fifo_tready(tx_fifo_tready), //input from TX FIFO
        .tx_fifo_tdata(tx_fifo_tdata) , //data to MAC
        .tx_fifo_tlast(tx_fifo_tlast), 
        .tx_fifo_tuser(tx_fifo_tuser),  // connect to tx fifo
        .arp_dv_in(arp_rx_dv_out), 
        .remote_mac(remote_mac), 
        .remote_ip(remote_ip),
        .udp_tvalid(buf_tx_tvalid), 
        .udp_tready(buf_tx_tready),  //output
        .udp_tdata(buf_tx_tdata), 
        .udp_tlast(buf_tx_tlast), 
        .udp_tuser(buf_tx_tuser),
        .length_tvalid(length_tvalid), 
        .length_tready(length_tready), //output
        .length_tdata(length_tdata)
    );
    ...
    totlen totlen_inst (
        .srst(srst),
        .clk(clk),
        .s_tvalid(udp_tx_tvalid), 
        .s_tready(udp_tx_tready), //output
        .s_tdata(udp_tx_tdata), 
        .s_tlast(udp_tx_tlast),
        .m_tvalid(buf_tx_tvalid), 
        .m_tready(buf_tx_tready), //input
        .m_tdata(buf_tx_tdata), 
        .m_tlast(buf_tx_tlast),
        .length_tvalid(length_tvalid), 
        .length_tready(length_tready), //input
        .length_tdata(length_tdata)
    );
